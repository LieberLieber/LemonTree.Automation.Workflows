...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="test-case">
		...
		<xsl:variable name="inner" select="exsl:node-set(//message/text())" />
		<xsl:variable name="EAFile">
			<xsl:value-of disable-output-escaping="yes" select="child::node()/message/EAFile"/>
		</xsl:variable>
		<xsl:variable name="EAWeb">
			<xsl:value-of disable-output-escaping="yes" select="child::node()/message/EAWeb"/>
		</xsl:variable>
		...
		<li>
			<xsl:choose>
				...
				<xsl:when test="$type='Inconclusive '">
					<span class="inconclusive ">
						<xsl:value-of select="$type" />
					</span>			
				</xsl:when>
			</xsl:choose>
